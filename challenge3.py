"""
    The hint picture actually asks us to find patterns in source string like:
    LHHHLHHHL (Be careful about the word EXACTLY)
    where L means lower case letter, and H means higher case letter.

    When we are trying to replace equality with linkedlist, which is the result,
    we will get linkedlist.php, which indicates we should replace equality.html as a whole.
"""
...
for i in range(height):
    for j in range(3, width - 3):
        if lines[i][j].islower() and \
            (j == 3 or lines[i][j - 4].islower()) and \
            not lines[i][j - 3].islower() and \
            not lines[i][j - 2].islower() and \
            not lines[i][j - 1].islower() and \
            not lines[i][j + 1].islower() and \
            not lines[i][j + 2].islower() and \
            not lines[i][j + 3].islower() and \
            (j == width - 4 or lines[i][j + 4].islower()):
